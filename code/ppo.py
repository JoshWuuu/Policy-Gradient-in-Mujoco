...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        # 1. Compute the log probabilities of the actions given the observations.
        log_probs = self.policy.action_distribution(observations).log_prob(actions)
        # 2. Compute the ratio between the new and old log probabilities.
        ratios = torch.exp(log_probs - old_logprobs)
        # 3. Compute the surrogate loss.
        surr1 = ratios * advantages
        # 4. Compute the clipped surrogate loss.
        surr2 = torch.clamp(ratios, 1-self.eps_clip, 1+self.eps_clip) * advantages
        # 5. Compute the PPO loss.
        loss = -torch.min(surr1, surr2).mean()
        # 6. Perform a gradient descent step on the PPO loss.
